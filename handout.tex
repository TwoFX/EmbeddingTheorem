...
\begin{document}
	...
	\section{Der Einbettungssatz}
		...
		\p{\begin{proof}
			...
			\textcolor{red}{Das Buch schreibt jetzt \enquote{Da $F$ linksexakt ist und nur endlich
			viele $D$ vorkommen, korrespondiert die Familie $a_D$ mit einem Element $a\in F(\lim\Gamma(D))$.}
			Hier geht es offenbar darum, dass $F$ endliche Limiten erhält, aber mir ist vollkommen schleierhaft,
			was hier passiert und
			wie wir an das $a$ kommen und was es bedeutet, sodass ich in dem Beweis nicht weiter fortschreiten kann.}

			\textcolor{red}{Abgesehen davon ist mir nicht klar, an welcher Stelle der Beweis erzwingt,
			dass das $\alpha_{D_0}$, das in dem Paar $(\overline{S}, (\alpha_D)_{D\in\overline{S}})$ vorkommt,
			mit dem $\alpha_{D_0}$ aus der Voraussetzung übereinstimmt.}
		\end{proof}}
		...
		\p{\begin{proof}
			...
			in $\ccat{Mod}_R$ genauso funktionieren wie in $\ccat{Ab}$ \textcolor{red}{(achja?)},
			...
			\[
				\begin{tikzcd}[column sep=7em]
					\cat{A}(\pi D,{-}) \n
					\cat{A}(\phi D, {-})
						\ar[shift right, ']{l}{\cat{A}(u_D,{-})}
						\ar[shift left]{l}{\cat{A}(v_D,{-})} \n
					\cat{A}(A, {-})
						\ar[']{l}{\cat{A}(\phi(D\leq\delta_A),{-})}
				\end{tikzcd}
			\]
			...
			initial in $\cat{D}$ (ohne Beweis\textcolor{red}{, aber ich sollte zumindest
			präzisieren, was mit \enquote{initial} hier gemeint ist\dots{} Borceux macht das
			mit finalen Funktoren, geht das auch einfacher?}). Rechts steht
			...
			\textcolor{red}{Hier geht es dann Montagnachmittag weiter\dots}
		\end{proof}}
\end{document}
